...
function registerjsonschema(name, schema) {
	...
	if (!schemaid && typeof(schema) === 'string') {
		if ((/:|,/).test(schema)) {
			...
			// Tries to create from the snipkode Schema
...
HEADERS.sse['X-Powered-By'] = 'snipkode';
...
HEADERS.file_lastmodified['X-Powered-By'] = 'snipkode';
...
HEADERS.file_release_compress['X-Powered-By'] = 'snipkode';
...
HEADERS.file_release_compress_range['X-Powered-By'] = 'snipkode';
...
HEADERS.file_release['X-Powered-By'] = 'snipkode';
...
HEADERS.file_release_range['X-Powered-By'] = 'snipkode';
...
HEADERS.file_debug_compress['X-Powered-By'] = 'snipkode';
...
HEADERS.file_debug_compress_range['X-Powered-By'] = 'snipkode';
HEADERS.file_debug = {};
...
HEADERS.file_debug['X-Powered-By'] = 'snipkode';
...
HEADERS.file_debug_range['X-Powered-By'] = 'snipkode';
...
HEADERS.content_mobile_release['X-Powered-By'] = 'snipkode';
...
HEADERS.content_mobile['X-Powered-By'] = 'snipkode';
...
HEADERS.content_compress['X-Powered-By'] = 'snipkode';
HEADERS.content = {};
...
HEADERS.content['X-Powered-By'] = 'snipkode';
...
HEADERS.stream_release_compress['X-Powered-By'] = 'snipkode';
...
HEADERS.stream_release['X-Powered-By'] = 'snipkode';
...
HEADERS.stream_debug_compress['X-Powered-By'] = 'snipkode';
...
HEADERS.stream_debug['X-Powered-By'] = 'snipkode';
...
HEADERS.binary_compress['X-Powered-By'] = 'snipkode';
HEADERS.binary = {};
HEADERS.binary[HEADER_CACHE] = 'public';
HEADERS.binary['X-Powered-By'] = 'snipkode';
...
HEADERS.response503ddos['X-Powered-By'] = 'snipkode';
...
function Framework() {
	...
	global.CONF = {
		...
		'security.txt': 'Contact: mailto:support@snipkode.com\nContact: https://www.snipkode.com/contact/',
		etag_version: '',
		directory_src: '/.src/',
...
var EMIT2MESSAGE = { TYPE: 'snipkode:emit' };
...
F.serverless = function(req, res, callback, types, cwd) {
	...
	else if (process.env.issnipkodeworker)
...
require('snipkode/{1}')(options);`.format(socket.replace(/\\/g, '\\\\'), DEBUG ? 'debug' : 'release', item, (options.cluster === 'auto' ? '\'auto\'' : options.cluster) || 0);
...
function threadforkmessage(m) {
	if (m && m.TYPE === 'snipkode:emit' && m.name) {
...
F.console = function() {
	...
	console.log('snipkode      : v' + F.version);
...
function configure_configs(arr, rewrite) {
	...
	if (!CONF.directory_temp)
		CONF.directory_temp = '~' + U.path(Path.join(Os.tmpdir(), 'snipkode' + F.directory.hash(true).toString(36)));
...
global.TotalAPI = function(token, type, data, callback, filename) {
	...
	var opt = {};
	opt.method = 'POST';
	opt.url = 'https://api.snipkode.com/' + type + '/';
	...
	for (var i = 0; i < arguments.length; i++) {
		...
		if (filename[0] === 'l' && filename[9] === 'd' && filename.substring(0, 10) === 'livereload') {
			if (DEBUG) {
				...
				builder += '<script src="//cdn.snipkode.com/livereload.js"' + (url ? (' data-url="' + url + '"') : '') + '></script>';
			}
			continue;
		}
...
function parseComponent(body, filename) {
	...
	while (true) {
		beg = body.indexOf('<script type="text/snipkode">');
		if (beg === -1) {
			beg = body.indexOf('<script total>');
			if (beg === -1)
				beg = body.indexOf('<script snipkode>');
			if (beg === -1)
				break;
		}
...
global.ACTION = function(url, data, callback) {
	...
	req.headers = { 'user-agent': 'snipkode/v' + F.version_header, 'x-test': 1 };
	req.uri = framework_internal.parseURI(req);
	req.path = framework_internal.routesplit(req.uri.pathname);
	req.method = method;
