...
- Fixed CLI List Templates
- Fixed CLI Install Packages
- Fixed CLI Import Module
...
- added support for WebSocket for the snipkode Proxy
...
- extended `Icon` schema type by adding support for snipkode Icons
...
- fixed a default nullable value for `enum` types in the snipkode schemas
...
- added `F.frameworkless(debug, options, [callback])` that executes HTTP server without snipkode dependencies
...
- added new command `CMD('importbuild', 'URL address', [callback(err)])` method for importing snipkode Builds
...
- updated Tangular version
- fixed `--minify` argument in the snipkode command-line utility
...
- added `NEWMACRO(script)` method for compiling custom snipkode macros
...
- fixed converting snipkode Schema to the JSON Schema
- improved error handling in JSON schemas
...
- added `OPENCLIENT()` method that simplifies work with snipkode Open products
...
- improved bundle generator in the `$ snipkode` executable
...
- added `options.servicemode` to the start script for running snipkode app without HTTP server
- fixed error status codes
- fixed default root [#24](https://github.com/snipkode/framework4/issues/24)
- fixed JavaScript compressor [#25](https://github.com/snipkode/framework4/issues/25)
...
- added a support for editing local files via snipkode Code Editor
- extended snipkode terminal app by adding `snipkode edit URL_TO_CODE_EDITOR?id=projectid`
...
- added `TMSCLIENT(url, [token], [callback(client)])` that creates a connection to the snipkode Message Service server
- extended snipkode Message Service by adding `call(name, data, callback)` functionality
...
- improved snipkode 4 command-line tool by [Helferino](https://github.com/Helferino/)
...
- fixed sending emails via snipkode API services
...
- added `PAUSE(is_paused)` method that can pause loading of all snipkode dependencies
